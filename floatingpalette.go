...
func WithFloatingPaletteTheme(theme string) FloatingPaletteOption {
	return func(fp *FloatingPalette) {
		fp.applyDesignTokens(designTokensForTheme(theme))
	}
}
...
func NewFloatingPalette(commands []PaletteCommand, opts ...FloatingPaletteOption) *FloatingPalette {
	fp := &FloatingPalette{
		...
		horizontalMargin: 8,
		prompt:           "> ",
		promptColor:      ansiReset,
		descriptionColor: ansiDim,
		selectedColor:    ansiInverse,
	}
...
func (fp *FloatingPalette) Update(msg tea.Msg) (Component, tea.Cmd) {
	switch msg := msg.(type) {
...
func (fp *FloatingPalette) SetCommands(cmds []PaletteCommand) {
	...
	if fp.selectedIndex >= len(fp.filtered) {
		fp.selectedIndex = max(0, len(fp.filtered)-1)
	}
}
...
func (fp *FloatingPalette) renderInputLine(contentWidth int) string {
	...
	prefix := fp.promptColor + fp.prompt + ansiReset
	...
	cursor = ansiInverse + cursor + ansiReset
	...
	if lineWidth > contentWidth {
		...
		return truncateANSI(line, contentWidth-3) + "..."
	}
	if lineWidth < contentWidth {
...
func (fp *FloatingPalette) renderResults(contentWidth int) []string {
	limit := min(len(fp.filtered), fp.maxResults)
	if limit <= 0 {
		return nil
	}
	...
	for i := 0; i < limit; i++ {
		cmd := fp.filtered[i]
		line := fmt.Sprintf(" %-10s %s%s%s", cmd.Name, fp.descriptionColor, cmd.Description, ansiReset)
		if cmd.Shortcut != "" {
			line += fmt.Sprintf(" %s%s%s", ansiDim, cmd.Shortcut, ansiReset)
		}
		...
		if lineWidth > contentWidth {
			if contentWidth <= 3 {
				...
				line = truncateANSI(line, contentWidth-3) + "..."
			}
		...
		if i == fp.selectedIndex {
			line = fp.selectedColor + ansiBold + line + ansiReset
		}
...
func (fp *FloatingPalette) applyDesignTokens(tokens *design.DesignTokens) {
	...
	if fg := ansiColorFromHex(tokens.Color); fg != "" {
