...
func TestProgressBarNonGradientRendering(t *testing.T) {
	...
	if !strings.Contains(view, ansiGreen) && !strings.Contains(view, "\x1b[38;2;") {
		t.Fatalf("expected non-gradient filled color in output, got %q", view)
	}
	if !strings.Contains(view, ansiDim) {
