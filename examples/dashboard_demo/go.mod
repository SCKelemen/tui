module github.com/SCKelemen/tui/examples/dashboard_demo

go 1.25.4

require (
	github.com/SCKelemen/tui v0.0.0
	github.com/charmbracelet/bubbletea v1.3.10
)

require (
	github.com/SCKelemen/cli v0.0.0 // indirect
	github.com/SCKelemen/color v1.0.0 // indirect
	github.com/SCKelemen/design-system v0.1.0 // indirect
	github.com/SCKelemen/layout v1.1.0 // indirect
	github.com/SCKelemen/text v0.0.0 // indirect
	github.com/SCKelemen/unicode v1.0.1-0.20251225190048-233be2b0d647 // indirect
	github.com/SCKelemen/units v1.0.2 // indirect
	github.com/atotto/clipboard v0.1.4 // indirect
	github.com/aymanbagabas/go-osc52/v2 v2.0.1 // indirect
	github.com/charmbracelet/bubbles v0.21.0 // indirect
	github.com/charmbracelet/colorprofile v0.2.3-0.20250311203215-f60798e515dc // indirect
	github.com/charmbracelet/lipgloss v1.1.0 // indirect
	github.com/charmbracelet/x/ansi v0.10.1 // indirect
	github.com/charmbracelet/x/cellbuf v0.0.13-0.20250311204145-2c3ea96c31dd // indirect
	github.com/charmbracelet/x/term v0.2.1 // indirect
	github.com/clipperhouse/uax29/v2 v2.2.0 // indirect
	github.com/erikgeiser/coninput v0.0.0-20211004153227-1c3628e74d0f // indirect
	github.com/lucasb-eyer/go-colorful v1.2.0 // indirect
	github.com/mattn/go-isatty v0.0.20 // indirect
	github.com/mattn/go-localereader v0.0.1 // indirect
	github.com/mattn/go-runewidth v0.0.19 // indirect
	github.com/muesli/ansi v0.0.0-20230316100256-276c6243b2f6 // indirect
	github.com/muesli/cancelreader v0.2.2 // indirect
	github.com/muesli/termenv v0.16.0 // indirect
	github.com/rivo/uniseg v0.4.7 // indirect
	github.com/xo/terminfo v0.0.0-20220910002029-abceb7e1c41e // indirect
	golang.org/x/sys v0.39.0 // indirect
	golang.org/x/term v0.38.0 // indirect
	golang.org/x/text v0.3.8 // indirect
)

replace (
	github.com/SCKelemen/cli => ../../../cli
	github.com/SCKelemen/text => ../../../text
	github.com/SCKelemen/tui => ../..
	github.com/SCKelemen/wpt-test-gen => ../../../wpt-test-gen
)
