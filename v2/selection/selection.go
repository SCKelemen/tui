...
func WithSelectionTheme(theme string) SelectionOption {
	return func(sm *SelectionManager) {
		sm.designTokens = style.DesignTokensForTheme(theme)
	}
}
...
func (sm *SelectionManager) StyledLine(line string, row int) string {
	...
	if sm.designTokens != nil {
		fg := style.ANSIColorFromHex(sm.designTokens.Color)
		bg := style.ANSIBackgroundColorFromHex(sm.designTokens.Accent)
		if bg != "" {
			onCode = fg + bg
			offCode = "\033[39m\033[49m"
		}
	}
