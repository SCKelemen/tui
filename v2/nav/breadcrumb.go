...
func WithBreadcrumbTheme(theme string) BreadcrumbOption {
	return func(b *Breadcrumb) {
		b.designTokens = style.DesignTokensForTheme(theme)
	}
}
...
func (b *Breadcrumb) Update(msg tea.Msg) (tui.Component, tea.Cmd) {
	switch msg := msg.(type) {
...
func (b *Breadcrumb) View() string {
	...
	if b.designTokens != nil {
		if c := style.ANSIColorFromHex(b.designTokens.Accent); c != "" {
			accent = c + style.ANSIBold
		}
	}
	dim := style.ANSIDim
	if b.designTokens != nil {
		if c := style.ANSIColorFromHex(b.designTokens.Color); c != "" {
			dim = c + style.ANSIDim
		}
	}
	underline := style.ANSIUnderline
