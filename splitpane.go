...
func WithSplitPaneTheme(theme string) SplitPaneOption {
	return func(s *SplitPane) {
		s.designTokens = designTokensForTheme(theme)
	}
}
...
func WithLeftComponent(c Component) SplitPaneOption {
	return func(s *SplitPane) {
		s.left = c
	}
}
...
func WithRightComponent(c Component) SplitPaneOption {
	return func(s *SplitPane) {
		s.right = c
	}
}
...
func (s *SplitPane) Update(msg tea.Msg) (Component, tea.Cmd) {
	switch m := msg.(type) {
...
func (s *SplitPane) SetLeftComponent(c Component) {
	s.left = c
	s.syncPaneFocus()
...
func (s *SplitPane) SetRightComponent(c Component) {
	s.right = c
	s.syncPaneFocus()
	_ = s.updateChildSizes()
}
...
func (s *SplitPane) focusedChild() Component {
	if s.focusedPane == 0 {
		return s.left
	}
	return s.right
...
func (s *SplitPane) renderPaneLines(c Component, width, height int) []string {
	if width < 0 {
		width = 0
	}
	if height < 0 {
...
func (s *SplitPane) dividerVisual() string {
	...
	if s.designTokens != nil {
		accent = ansiColorFromHex(s.designTokens.Accent)
	}
	if accent != "" {
		return accent + ch + ansiReset
	}
	return ansiInverse + ch + ansiReset
}
